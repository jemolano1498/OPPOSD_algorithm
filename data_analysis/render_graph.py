...
def onclick(event):
    global pacing
    pacing = not(pacing)
    print('Pace %s' % ('Activated' if pacing else 'Dis-activated'))
...
def animate(iFrame):
    ...
    x=x+1
    current_pace, new_state, reward, real_pacing, done = env.step(pacing, pref_pace)
    total_reward = total_reward + reward
    x_acc = np.append(x_acc, x)
    state = np.append(state, (new_state[0]+1)*pref_pace)
    pace_color = 'b'
    ...
    plt.annotate("Avg. Pace: %s"%(np.round((new_state[0]+1)*pref_pace, 0)), xy=(100, 1), xycoords='figure points',
                 bbox=dict(fc='white', ec='white'))
